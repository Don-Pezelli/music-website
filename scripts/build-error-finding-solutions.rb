...
scores.each do |number,bars|
 ...
 svg=%(<svg xmlns="http://www.w3.org/2000/svg" width="960" height="145" viewBox="0 0 960 145" role="img" aria-labelledby="title desc"><title id="title">Korrekte Notation zu Fehler hören und finden #{number}: ein Takt im 4/4-Takt</title><desc id="desc">#{CGI.escapeHTML(description)}</desc>#{parts.join}</svg>)
 raise 'Forbidden rest' if svg.include?('E4E7')
 File.write(File.join(base,"assets/rhythmus-2/fehler-finden-loesung-#{number}.svg"),svg)
end
