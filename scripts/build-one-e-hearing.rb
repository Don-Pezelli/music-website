...
exercises={
 1=>{notes:{0=>2,1=>2},rests:[[0.5,0.5],[1.5,0.5],[2,2]]},
 2=>{notes:{0=>4,3=>2},rests:[[1,1],[2,1],[3.5,0.5]]},
 3=>{notes:{1=>4,2=>4,3=>2},rests:[[0,1],[3.5,0.5]]}
}
exercises.each do |number,exercise|
 ...
 exercise[:notes].each do |beat,count|
  stems=[]
  count.times do |i|
   x=130+beat*210+i*52.5; stem=x+11.3; stems << stem
   parts << %(<path data-smufl="E0A4" data-onset="#{beat+i*0.25}" data-duration="0.25" fill="black" transform="translate(#{x} 120.8) scale(0.52 -0.52)" d="#{glyphs.fetch('E0A4')[:path]}"/>)
 ...
 File.write(File.join(base,"assets/rhythmus-2/hoerloesung-1e-#{number}.svg"),svg)
end
