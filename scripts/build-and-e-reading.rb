...
(1..4).each do |exercise|
 ...
 4.times do |beat|
  labels=beat==active ? [(beat+1).to_s,'e','&amp;','e'] : [(beat+1).to_s]
 ...
 stems=[]
 2.times do |i|
  x=235+active*210+i*52.5; stem=x+11.3; stems<<stem
 ...
 parts << %(<path data-smufl="E4E6" data-duration="0.5" fill="black" transform="translate(#{130+active*210} 120.8) scale(0.52 -0.52)" d="#{glyphs.fetch('E4E6')[:path]}"/>)
 rests={1=>[[1,1],[2,2]],2=>[[0,1],[2,2]],3=>[[0,2],[3,1]],4=>[[0,2],[2,1]]}.fetch(exercise)
 ...
 svg=%(<svg xmlns="http://www.w3.org/2000/svg" width="960" height="145" viewBox="0 0 960 145" role="img" aria-labelledby="title"><title id="title">Leseübung #{exercise}: zwei Sechzehntel auf der zweiten Hälfte von Schlag #{exercise}, Pausen auf den übrigen Schlägen</title>#{parts.join}</svg>)
 raise 'Wrong total duration' unless svg.scan(/data-duration="([\d.]+)"/).flatten.map(&:to_f).sum==4
 File.write(File.join(base,"assets/rhythmus-2/leseuebung-ande-#{exercise}.svg"),svg)
end
