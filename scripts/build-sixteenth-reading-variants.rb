...
(2..4).each do |exercise|
 ...
 rests={2=>[[0,1],[2,2]],3=>[[0,2],[3,1]],4=>[[0,2],[2,1]]}.fetch(exercise)
 ...
 svg=%(<svg xmlns="http://www.w3.org/2000/svg" width="960" height="145" viewBox="0 0 960 145" role="img" aria-labelledby="title"><title id="title">Leseübung #{exercise}: vier Sechzehntel auf Schlag #{exercise}, Pausen auf den übrigen Schlägen</title>#{parts.join}</svg>)
 raise 'Wrong total duration' unless svg.scan(/data-duration="([\d.]+)"/).flatten.map(&:to_f).sum==4
 File.write(File.join(base,"assets/rhythmus-2/leseuebung-#{exercise}.svg"),svg)
end
