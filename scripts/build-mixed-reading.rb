...
scores.each do |number,bars|
 ...
 svg=%(<svg xmlns="http://www.w3.org/2000/svg" width="960" height="300" viewBox="0 0 960 300" role="img" aria-labelledby="title desc"><title id="title">Gemischte Leseübung #{number}: vier Takte im 4/4-Takt</title><desc id="desc">#{CGI.escapeHTML(description)}</desc>#{parts.join}</svg>)
 raise 'Forbidden rest' if svg.include?('E4E7')
 File.write(File.join(base,"assets/rhythmus-2/lesen-gemischt-#{number}.svg"),svg)
end
puts 'Generated 3 exercises with 4 complete bars each; only permitted undotted values.'
